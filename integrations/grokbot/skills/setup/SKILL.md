...
先发现实际可执行文件和连接方式。将选定入口展开为绝对路径并解析符号链接，核对目标文件的安装来源和 `version` 返回，再把真实文件路径保存为 binding 的 `--asterun`。比较它与核心 `diagnose` 返回的版本，并读取所选后端的 `backend-inspect`。`--ledger` 使用接收者私有的真实目录；入口或账本路径触发 `PATH_INVALID` 时，先处理具体路径错误。安装、登录、实际可调用和客户端可见分别核验。已有正确配置保留，调整时使用配置修订和既有备份恢复流程。
...
确认配套代码及运行包已安装后，按[配套 CLI](https://github.com/zonzideka/asterun/blob/grokbot-template-v0.1.0-rc1/integrations/grokbot/README.md#配套-cli)使用 `init` 保存私有绑定，指定 `--ledger`、已解析的 Asterun 真实文件路径、核心配置、状态目录、执行位置、允许的工作区与后端。为每个接收聊天生成并保存唯一的私有逻辑别名，作为 `--destination PRIVATE_CHAT_ALIAS` 的值。别名用于账本关联，原聊天由当前对话和 routine 归属确认，发送使用该聊天中的 `SendToUser`。当前宿主工具未提供稳定的原生聊天 ID，发送也无需传入这个别名。云电脑使用 `--location host-cloud`，本机使用 `--location local`。只有 `init` 返回 `ok=true` 后才执行 `submit`；失败时先处理 Envelope 中的具体 `error`。后端登录与调用按实际结果记录。同名绑定内容有变化时，使用新 binding，保留旧任务关系。
...
发送入口为 `SendToUser`。仅在 `claim` 返回 `ok=true`、`data.already_claimed=false`、`data.delivery_state=sending` 时，使用 `type=text`、`end_turn=false` 原样发送 `data.content`。在当前轮次用宿主返回的消息 ID 与 `data.content_sha256` 完成 `ack`，记录 `host_reported_delivered`。随后再次运行 `poll`，再读 `status`；`needs_followup=false` 时通过 `update_state` 的 `action=pause` 暂停 routine，再结束本轮。新任务提交后恢复。完整过程见[任务技能](https://github.com/zonzideka/asterun/blob/grokbot-template-v0.1.0-rc1/integrations/grokbot/skills/tasks/SKILL.md)。

云电脑已有材料复制、摘要校验、固定 a13 wheel 下载、Python 3.13.5 独立 venv 安装与版本读取记录。fake 任务已验证相同请求键复用任务、离开聊天页面后的 cron 唤醒、原聊天交付、claim/ack、末次 poll 返回 `needs_followup=false` 及暂停 routine。本机 Grok Build 真实短文任务及摘要核对已通过，宿主回执已记录；其原聊天 UI 显示、云端真实模型、应用重启恢复和独立接收方继续按[验收记录](https://github.com/zonzideka/asterun/blob/grokbot-template-v0.1.0-rc1/integrations/grokbot/acceptance.md)逐项验证；同账户 Bot 副本保留原账户环境。
...
Discover the actual executable and connection method. Expand the selected entry to an absolute path and resolve symbolic links, verify the target file's installation source and `version` response, then save that real file path as the binding's `--asterun`. Compare its version with core `diagnose`, then read `backend-inspect` for the selected backend. Use a real private directory for the recipient's `--ledger`. If an executable or ledger path produces `PATH_INVALID`, address the specific path error first. Check installation, login, live invocation, and client visibility separately. Preserve correct configuration; use configuration revisions and the existing backup and recovery procedures for changes.
...
After installing the supporting code and runtime, follow the [adapter CLI](https://github.com/zonzideka/asterun/blob/grokbot-template-v0.1.0-rc1/integrations/grokbot/README.en.md#adapter-cli) to save a private binding with `init`. Specify `--ledger`, the resolved real Asterun file path, core configuration and state directory, execution location, and allowed workspaces and backends. Generate and save a unique private logical alias for each receiving chat as the value of `--destination PRIVATE_CHAT_ALIAS`. The alias associates ledger records; the current conversation and routine ownership identify the original chat, where `SendToUser` sends. The available host tools do not expose a stable native chat ID or require this alias for sending. Use `--location host-cloud` for the cloud computer or `--location local` for local execution. Run `submit` only after `init` returns `ok=true`; otherwise address the Envelope's specific `error` first. Record backend login and invocation from actual results. If a named binding changes, create a new binding and preserve old task associations.
...
Send through `SendToUser` only after `claim` returns `ok=true`, `data.already_claimed=false`, and `data.delivery_state=sending`. Set `type=text`, `end_turn=false`, and send `data.content` unchanged. In the same turn, use the host's returned message ID and `data.content_sha256` in `ack` to record `host_reported_delivered`. Then run `poll` again and read `status`. When `needs_followup=false`, pause the routine with `update_state`, `action=pause`, before ending the turn; resume after a new submission. See the [task skill](https://github.com/zonzideka/asterun/blob/grokbot-template-v0.1.0-rc1/integrations/grokbot/skills/tasks/SKILL.md) for the complete sequence.

Cloud installation records cover copying material, verifying digests, downloading the pinned a13 wheel, installing it in a separate Python 3.13.5 venv, and reading its version. A fake task has verified reuse of a request key, a cron wake after navigating away from the chat page, delivery to the original chat, claim/ack, a final poll with `needs_followup=false`, and routine pause. Local Grok Build has completed a real text task with a checked summary and a recorded host receipt. Complete UI confirmation of its result, cloud model invocation, application-restart recovery, and independent recipient checks through the [acceptance record](https://github.com/zonzideka/asterun/blob/grokbot-template-v0.1.0-rc1/integrations/grokbot/acceptance.md). Bot copies under the same account retain that account's environment.
