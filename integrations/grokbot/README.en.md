...
Open the [native Asterun template](https://x.ai/bot/4r3ysO1eA-6ZLI6cRoTNU). The sharing page states that clicking Add to Grok Bot accepts the [sharing terms](https://x.ai/legal/bot-sharing-terms); the user confirms this when importing. Then follow the setup instructions to obtain the supporting code bundle below.
...
Local Grok Build has completed a text task in the real project directory, and its summary has been checked against the input. The host receipt records `host_reported_delivered`, and the final poll requires no follow-up. Subsequent native UI checks confirmed the result, receipt, and summary in the original chat, along with all three temporary routines being paused. Version 3 of the native template has verified all 11 memory entries and 3 skills in full, along with the published card and public page. Same-account import, cloud model invocation, application-restart recovery, and independent recipient acceptance remain in the [acceptance record](acceptance.md). Bot copies and cloud computers under the same account retain that account's environment; independent new-user verification is still pending. See the [release notes](https://github.com/zonzideka/asterun/blob/v0.1.0a13/docs/en/release-v0.1.md) for backend support.

The fixed release ZIP is public and has passed anonymous-download, checksum, and complete manifest checks. Its documentation retains the record at release; the current acceptance documents separately record the later local UI, routine-pause, and native-template publication confirmations.
