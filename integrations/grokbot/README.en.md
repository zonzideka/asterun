...
These interfaces are provided by `scripts/bridge.py`. First enter the template root saved in the setup record; commands do not depend on the host skill library's current directory: enter `integrations/grokbot` in a full Git checkout, or the extracted template directory in a shared bundle. Replace paths, backend and workspace aliases, and destination with the recipient's verified values. The adapter uses the standard library. `--ledger` selects this instance's private tracking records. The commands below return one JSON Envelope on stdout.
...
For cloud execution, use `--location host-cloud`. Repeat `--workspace` and `--backend` on `init` as needed. `submit --input -` reads stdin. Add `--conversation-id` for continuation, subject to Asterun's binding checks. Retain `REQUEST_ID` for retries. Obtain and fix the chat destination through the current host interface.

Use `claim` to acquire a specific message. Send only when the Envelope has `ok=true` and `data.already_claimed=false`. Call `SendToUser` with `type=text` and `content` taken unchanged from `claim`'s `data.content`. After receiving an actual message ID from the host, record it with `ack`:
...
Keep the routine in the original chat with the same `binding`. Each wake runs one `poll --limit 3` on the bound execution host, then reads `inbox --limit 3`. Process that batch using `claim`, `SendToUser`, and `ack` as described above. Stay quiet while ordinary state is unchanged, then end the observation and wait for the next wake.
...
Validate wiring with an offline task in a fresh environment, then verify the real backend, later routine wakeups, same-chat delivery, and recovery within the authorized scope. Independent recipient acceptance is pending. Verified steps include copying a three-file installation bundle to the cloud computer and checking its SHA, downloading the pinned a13 wheel, installing it in a separate Python 3.13.5 venv, and reading its version. Backend invocation, complete task delivery, recovery after closing the chat, and independent recipient acceptance still require scenario-specific evidence. See the [release notes](https://github.com/zonzideka/asterun/blob/v0.1.0a13/docs/en/release-v0.1.md) for backend support.
