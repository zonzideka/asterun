...
The optional [GrokBot template](integrations/grokbot/README.en.md) is undergoing `0.1.0-rc1` acceptance. It tracks ordinary tasks through a separate SQLite ledger and host routines. All 56 template offline tests, 34 core packaging checks, and 10 repeated concurrent-submission checks pass; core runtime code and existing protocols are unchanged. The current account has verified cloud installation and a fake task's routine wake after navigating away from the chat, delivery to the original chat, acknowledgement, and routine pause. Local real-backend setup failed on a symbolic-link executable entry before any model task was created. Real-backend invocation, application restart, native template sharing, and independent new-user acceptance remain pending. See the [template acceptance record](integrations/grokbot/acceptance.md).
